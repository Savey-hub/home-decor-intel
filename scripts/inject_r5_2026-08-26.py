"""Idempotent injector for round-5 (2026-08-26, WebSearch+WebFetch public sources).

Reads _work/inject_r5_payload.json and merges into the four data JSONs.
Backs up each target to _work/bak_<name>_before_r5.json first.
(testResults are handled separately by backfill_testresults_r5.py.)
"""
import json
import os
import shutil
...
def main():
    payload = load(os.path.join(WORK, 'inject_r5_payload.json'))

    data = {}
    for tag, path in TARGETS.items():
        bak = os.path.join(WORK, 'bak_%s_before_r5.json' % os.path.basename(path).replace('.json', ''))
    ...
    total += add_unique(pd['platforms']['kuaishou'], payload.get('platform_kuaishou', []), ('title',), 'platforms.kuaishou')
