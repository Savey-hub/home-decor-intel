"""Idempotent injector for round-3 (2026-08-13, Quark browser) harvested intelligence.

Reads _work/inject_r3_payload.json and merges into the four data JSONs.
Backs up each target to _work/bak_<name>_before_r3.json first.
(testResults are handled separately by backfill_testresults_r3.py.)
"""
import json
import os
import shutil
...
def main():
    payload = load(os.path.join(WORK, 'inject_r3_payload.json'))

    data = {}
    for tag, path in TARGETS.items():
        bak = os.path.join(WORK, 'bak_%s_before_r3.json' % os.path.basename(path).replace('.json', ''))
