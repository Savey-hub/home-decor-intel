#!/bin/bash
# 每周三 10:00 由 qoder_cron 触发的刷新入口。
# 逻辑：
#   1) 由 agent 侧完成全板块采集与 JSON 更新（cron payload.message 会驱动）；
#   2) 本脚本负责重新 build 出 index.html，并把最新版拷到 outputs/；
#   3) 创建本地提交后，通过 GitHub Data API 发布。
set -e
...
PY="${QODER_PYTHON:-$HOME/.qoderwork/bin/python312/python.exe}"
