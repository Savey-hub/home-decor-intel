...
本轮通过授权浏览器 Computer Use 桌面自动化,重新采集了4个强登录/受限源:
...
cm = find('蝉妈妈')
if cm:
    cm['login'] = '强登录(授权浏览器·品牌版账号·本轮授权访问)'
    cm['depth'] = 3
...
jzt = find('京准通')
if jzt:
    jzt['login'] = '强登录(授权浏览器·授权账号账号·本轮已成功登录)'
    jzt['depth'] = 0
    jzt['timestamp'] = '2026-07-30(已登录,撞权限门槛)'
    jzt['blocker'] = ('本轮已用授权账号(已移除账号标识)成功登录京准通数据中心,但『行业分析(行业大盘/竞争分析/流量解析)』为权限门槛功能:'
...
jm = find('京麦')
if jm:
    jm['blocker'] = '本轮仍未获POP商家账号:授权为买家号提示『该账号暂不支持开店』,京麦商智家居家装类目数据未取,待用户以POP商家账号授权访问补采。'
...
summ['newThisRound'] = ('本轮(2026-07-30)强登录源陪跑重采:经授权浏览器 Computer Use 桌面自动化,'
...
summ['blockedNote'] = ('仍阻塞源:京麦(无POP商家权限,买家号不支持开店)、京准通行业分析(需有竞价投放的广告主账号)、微信文档(公司网络访问策略拦截)、'
...
if not has_title(plat, '蝉妈妈'):
    plat.append({
        ...
        "detail": ("授权浏览器现采蝉妈妈品牌库(带货分类=家具建材,近30天):按类目销售额降序TOP17依次为全友家居(731万)、林氏家居(421万)、"
...
if not has_title(plat, '千瓜'):
    plat.append({
        ...
        "detail": ("授权浏览器现采千瓜尊享版品牌排行榜(小红书·家居家装·商单笔记榜·月榜2026-06,按互动总量降序):TOP19中日化/个护品牌占13席"
...
if not has_title(plat, '阿拉丁') and not has_title(plat, '照明'):
    plat.append({
        ...
        "detail": ("授权浏览器现采阿拉丁照明网(照明行业第一门户)2026-07-30头条:温其东《2026上半年中国照明出口情况详解》、"
        ...
        "source": "阿拉丁照明网(授权浏览器现采,过WAF滑块)",
        "cat": "灯具光源·照明"
    })

# monthlySummary 补一句强登录重采说明
h['monthlySummary'] = h['monthlySummary'].rstrip() + (
    ' 【07-30强登录源重采】经授权浏览器陪跑,补齐抖音电商(蝉妈妈家具建材TOP17)、小红书(千瓜家居家装商单榜)、'
