...
# =========================================================
ASOF = macro.get("asOf","2026-07-26")
WINDOW = "近30天（2026-06-27 至 2026-07-26）"
...
def _month_group_desc(key):
    ...
    if key=="policy":
        return ("政策端标准与国补双轮驱动：智能卫浴可靠性国标、住宅全案整装服务标准、508项国标同步推高质量与合规门槛，"
                "利好研发/服务投入高的头部品牌、加速中小出清；北京(07-23)、福建(07-22)地方国补密集扩围，"
                "首次将智能马桶/智能床/智能沙发等智能睡眠与软体家具纳入补贴，示范效应向全国传导。"
                "天猫招商与流量分配应向合规头部倾斜，并用足国补新增品类的流量红利。")
    if key=="merchant":
        return ("H1业绩预告披露季头部集体探底：定制家居（欧派预降50-60%、志邦转亏、索菲亚预降78-85%）、软体（慕思预降44-51%）、"
                "卫浴（箭牌转亏、惠达预亏）、瓷砖（东鹏净利降85-88%）、板材（大亚圣象预亏）全品类承压；"
                "但蒙娜丽莎扭亏、红星美凯龙扭亏、三棵树零售涂料韧性、东鹏逆势净增门店50家显示『收缩工程/做强零售』自救路径见效。"
                "运营需警惕定制/软体赛道承压对天猫GMV的传导，同时关注做强零售的头部品牌合作机会。")
    return ""
...
def section_sources(doc):
    ...
    desc(doc, f"共接入{sm['totalSources']}个数据源：深层采集(depth3){sm['depth3']}个、中层{sm['depth2']}个、浅层{sm['depth1']}个、"
              ...
              f"公开层已采集{sm['publicItemsCollected']}。本期(2026-07-30)经本机夸克浏览器 Computer Use 桌面自动化陪跑，"
              "已重采抖店罗盘、并首次采到蝉妈妈(抖音电商·家具建材品牌销售额TOP17)、千瓜尊享版(小红书·家居家装商单笔记榜TOP19)、"
              "阿拉丁照明网(照明行业2026H1景气资讯，过堡塔云WAF滑块)三大站外精确数据源；仍阻塞的为：京麦(无POP商家权限)、"
              "京准通行业分析(平台权限门槛，仅对有竞价广告投放的pin开放，该账号零投放故取不到)、微信文档(公司网络DLP拦截)、"
              "千瓜行业流量大盘曲线(需更高阶数据权限)——均非采集能力问题，已按阻塞透明规则标注，待相应账号/权限就绪即可补采。")
...
def section_conclusion(doc):
    ...
    for b in ["京麦商智：待用户切换POP商家账号扫码陪跑（本轮买家号提示『暂不支持开店』）",
              "京准通行业分析：待用户提供有实际竞价广告投放的京东广告主账号（本轮账号零投放，撞平台权限门槛）",
              "抖店罗盘剩余页（榜单/趋势/竞争分析）：能力可达，下期展开补采",
              "千瓜行业流量大盘曲线：需更高阶数据权限，本轮已改从品牌排行榜维度采到家居家装商单榜",
              "小红书千帆/蒲公英量化流量：需对应商业产品账号权限",
              "微信文档：公司网络DLP拦截，需用手机/非阿里网络访问"]:
