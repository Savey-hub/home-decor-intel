...
LABELS = {
    'ok_yield': '授权访问进入成功·有新增情报',
    'ok_yield_internal': '授权访问进入成功·有情报(受限环境,仅入本地非公开材料)',
    'ok_empty': '夸克进入成功·窗口内无新增',
    'ok_empty_internal': '授权访问进入成功·无可注入结构化数据(受限环境)',
    'partial': '夸克进入成功·口径不足部分可用',
    'dead': '链接失效·已确认',
    'need_login_browser': '需授权访问·夸克当前未登录',
    'noise': '噪声源·已剔除',
}
...
def main():
    ...
    for u in urls:
        ...
        tr['round'] = 4
        tr['method'] = '授权浏览器授权访问(自动输入+扫描码)逐个进入抓取'
        u['testResult'] = tr
        u['status'] = v
        updated += 1
    ...
    inv['auditSummary']['note'] = (
        '2026-08-13 第4轮：在授权访问重跑用户自有付费源与受限来源。'
        '公开可注入：蝉妈妈品牌库(切家具建材类目,一次Ctrl+A取全量约48品牌榜,蓝盒子/公牛/顾家前三)、'
        '千瓜红书版(家居家装关键词近30天笔记505万+/互动2.26亿/企业账号占27.99%)——均为用户自有第三方付费源,已注入公开看板。'
        '受限来源(魔镜家装行业控比七夕环比+10.7%、FBI 4847同族控比)有效但属受限环境GMV,按方案1仅沉淀本地非公开材料、不进公开仓；'
        'Rune为按需问答式AI Agent无固定榜单、受限来源与魔镜数据重叠均不入库。'
        '第3轮针对 29 个需浏览器/登录源改用本机夸克(自动输入+扫描码)逐个抓取的结论保留,每源明细见 urls[].testResult(round字段区分轮次)。')
