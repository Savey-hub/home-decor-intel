...
local Quark browser (夸克, 自动输入+scancode), one by one. This script writes the
...
LABELS = {
    ...
    'ok_yield_internal': '夸克进入成功·受限环境有数据(仅入本地非公开材料)',
    'ok_empty': '夸克进入成功·仅落地页/导航,无新增结构化数据',
    'ok_empty_internal': '夸克进入成功·内部访问限制授权状态过期未取数(仅非公开材料)',
...
R = {
    ...
    4:  ('ok_yield', True, '千瓜·工作台:夸克授权访问进入成功,抓到约24KB工作台数据(种草/达人/内容指标)'),
    ...
    11: ('ok_yield', True, '字节飞书wiki-1(抖音电商Q3甄选品牌&靶向商家新商政策·商家版):夸克二次进入渲染成功,抓到约9.9KB(LIGHT运营框架+入驻激励:入驻送5万千川红包/0-180天返千川券最高150万/中重奢首年免佣0.6%),文档明示"对外对商家使用",已注入platform_douyin'),
    ...
    26: ('ok_yield', True, '阿拉丁照明网:夸克二次进入过滑动验证后渲染成功,抓到约9.1KB照明行业资讯(含"两大LED巨头官宣涨价:昕诺飞最高涨15%/洲明科技9月5日起执行"、2026中国LED照明行业报告、7月照明出口快报),已注入industry(灯具光源)'),
    27: ('ok_yield', True, '36氪·家居搜索:夸克进入成功,抓到约2.3KB家居频道近期文章(2026-08多篇)'),
    28: ('ok_yield', True, '中国建筑装饰协会(中装新网):夸克二次进入过安全验证后渲染成功,抓到约14.8KB行业资讯(含"关于成立装配式装修工作领导小组的通知"、住建部上半年新开工城市绿道2450公里、2026年度建筑装饰行业信用评价、金隅天坛联手华为鸿蒙智家全场景智慧家装),已注入policy'),
    29: ('dead', False, '微信文档·表格:夸克进入被访问策略域名拦截(约35B),企业安全策略阻断'),
    ...
    50: ('need_login_browser', False, '巨量算数·算术报告:夸克进入触发验证/登录(约0.6KB),需授权访问'),
    51: ('ok_empty', False, '贝壳研究院:夸克进入约1.1KB,仅门户landing'),
    52: ('ok_yield', False, '克而瑞CRIC:夸克二次进入渲染成功,抓到约7.2KB(CRIC2026 AI数据智能体落地页,含新房37.08万㎡/二手14073套/商办6.31亿等指标),但页面为多城市Tab默认口径未标注城市,按数据真实性纪律仅记可达、数值不注入公开仓'),
    53: ('ok_yield', True, '抖店大学:夸克进入成功,抓到约3.0KB学习中心内容'),
    54: ('ok_empty_internal', False, 'Rune·竞对AI:夸克进入被重定向到统一登录中心,本轮访问授权授权状态过期未取数;受限来源仅入本地非公开材料'),
    55: ('ok_empty_internal', False, '魔镜·竞争管控·行业管控:夸克进入被重定向到统一登录中心,本轮访问授权授权状态过期未取数;受限来源仅入本地非公开材料'),
    56: ('ok_empty', False, '千瓜官网:夸克进入约1.0KB首页,真实数据经工作台(id4)取得'),
    57: ('ok_empty_internal', False, 'FBI·报表(view 4847):夸克进入页面标题已加载但正文0B(需登录/画布渲染),本轮未取数;受限来源仅入本地非公开材料'),
}
...
def main():
    ...
    inv['auditSummary']['note'] = (
        '2026-08-26 第5轮：应要求把 url_inventory 全部 57 个数据源逐一用授权浏览器'
        '(自动输入+扫描码,Ctrl+A剪贴板抓取+向下滚动二次抓)实测进入,不再用 WebSearch/WebFetch 代替。'
        ...
        '【二次重试后恢复(共4个)】首轮判失败/拦截、第二次进入夸克后渲染成功:'
        '字节飞书wiki-1(抖音电商Q3甄选品牌新商政策·商家版,已注入platform_douyin)、'
        '阿拉丁照明网(过滑动验证,含两大LED巨头涨价快讯,已注入industry灯具光源)、'
        '中国建筑装饰协会/中装新网(过安全验证,含装配式装修工作领导小组通知,已注入policy)、'
        '克而瑞CRIC(落地页可达,但多城市Tab口径未标注,按数据真实性纪律仅记可达、数值不注入)。'
        '【仍未通过·需授权访问(共4个)】蝉妈妈榜单、抖店放心购、小红书种草学、巨量算数——需在夸克内授权访问后再抓;'
        '【确认失效/被访问策略拦截(共4个)】字节飞书wiki-2(0B)、百家号作者×2(账号失效/无正文)、微信文档(访问策略域名拦截);'
        '【噪声源已剔除(共3个)】百度首页、微软研究院、英伟达(英文站与家装无关);'
        '受限来源(Rune/魔镜/FBI)夸克进入均被重定向统一登录中心,本轮访问授权授权状态过期未取数,按方案1仅沉淀本地非公开材料、不进公开仓。')
